...
public final class HttpApiServer {
    ...
    public HttpApiServer(SigmaRCON plugin, ChunkLagMonitor chunkLagMonitor, LastSeenManager lastSeenManager) throws IOException {
        ...
        register("/api/memory", new MemoryHandler(), apiToken, allowedOrigins);
        register("/api/inventory", new InventoryHandler(plugin), apiToken, allowedOrigins);
        register("/api/map", new MapHandler(plugin), apiToken, allowedOrigins);
        register("/api/moderation", new ModerationHandler(plugin), apiToken, allowedOrigins);
